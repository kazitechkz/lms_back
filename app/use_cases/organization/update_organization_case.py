...
class UpdateOrganizationCase(BaseUseCase[OrganizationRDTOWithRelated]):
    def __init__(self, db: AsyncSession):
        self.repository = OrganizationRepository(db)
        self.type_repository = OrganizationTypeRepository(db)

    async def execute(self, id: int, dto: OrganizationCDTO) -> OrganizationRDTOWithRelated:
    ...
    async def validate(self, id: int, dto: OrganizationCDTO):
        if len(dto.bin) != 12:
            raise AppExceptionResponse.bad_request("БИН должен содержать 12 цифр")
        # Проверка существования организации
        existing_organization = await self.repository.get(id=id)
        if existing_organization is None:
            raise AppExceptionResponse.not_found("Организация не найдена")

        # Проверка уникальности БИН (если БИН изменяется)
        if existing_organization.bin != dto.bin:
        ...
        # Проверка существования типа организации
